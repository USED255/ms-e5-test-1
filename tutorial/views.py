...
def calendar(request):
    ...
    today = datetime.now(tz_info).replace(hour=0, minute=0, second=0, microsecond=0)
...
def newevent(request):
    ...
    if request.method == "POST":
        ...
        # Required values
        if (
            (not request.POST["ev-subject"])
            or (not request.POST["ev-start"])
            or (not request.POST["ev-end"])
        ):
            context["errors"] = [
                {
