...
def get_calendar_events(token, start, end, timezone):
    ...
    events = requests.get(
        "{0}/me/calendarview".format(graph_url), headers=headers, params=query_params
    )
...
def create_event(token, subject, start, end, attendees=None, body=None, timezone="UTC"):
    ...
    if body:
        ...
        new_event["body"] = {"contentType": "text", "content": body}
    ...
    requests.post(
        "{0}/me/events".format(graph_url), headers=headers, data=json.dumps(new_event)
    )
